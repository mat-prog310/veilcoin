# web/blueprint.py complet avec toutes les routes
...
def blockchain_page():
    ...
    return render_template('blockchain.html', blocks=blocks, stats={'height': len(blocks), 'difficulty': 5})

@web_bp.route('/market')
def market_page():
    return render_template('market.html', 
                         price=round(pool.get_veil_price(), 4),
...
def api_balance(name):
    try:
        ...
        return jsonify({'balance_veil': w.balance})
    except:
        return jsonify({'balance_veil': 0})
...
def market_buy():
    try:
        ...
        return jsonify({'success': True, 'veil_received': veil_amount, 'new_balance': w.balance})
    except Exception as e:
        return jsonify({'success': False, 'error': str(e)})
...
def market_sell():
    try:
        ...
        return jsonify({'success': True, 'eur_received': eur_amount, 'new_balance': w.balance})
    except Exception as e:
        return jsonify({'success': False, 'error': str(e)})
...
def submit_block():
    try:
        ...
        return jsonify({'success': True, 'reward': 25, 'new_balance': w.balance})
    except Exception as e:
        return jsonify({'success': False, 'error': str(e)})
...
def api_stats():
    return jsonify({'height': 0, 'difficulty': 5})
