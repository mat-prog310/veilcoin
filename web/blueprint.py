...
def submit_block():
    ...
    try:
        ...
        now = time.time()
        session = mining_sessions.get(wallet, {'blocks_mined': 0})
        ...
        mining_sessions[wallet] = session
        
        blocks_left_this_hour = max(0, MINING_RATE_LIMIT - session['blocks_mined'])
...
def check_mining_farm(wallet_address):
    ...
    # Vérifier le nombre de blocs minés récemment
    last_block_time = session.get('last_block_time', 0)
    blocks_mined = session.get('blocks_mined', 0)
    
    # Si trop de blocs en peu de temps (anti-ferme)
    if blocks_mined >= MINING_RATE_LIMIT:
        if now - last_block_time < MINING_SESSION_DURATION:
            return False, f"❌ Trop de blocs ! Limite de {MINING_RATE_LIMIT} blocs/heure. Attendez {int((MINING_SESSION_DURATION - (now - last_block_time))/60)} minutes."
    
    # Vérifier le cooldown après session
    session_end = session.get('session_end', 0)
    if now < session_end:
        return False, f"❌ Cooldown actif. Réessayez dans {int((session_end - now)/60)} minutes."
