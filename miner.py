...
class VeilMiner:
    def __init__(self):
        ...
        self.blocks = 0
        self.t0 = 0
        self.diff = 4  # ⚠️ DIFFICULTÉ MODIFIÉE À 4 ZÉROS
        self.bal = 0.0
    ...
    def stats(self):
        ...
        if r: 
            self.diff = r.get("difficulty", 4)  # Valeur par défaut à 4
        return r
    ...
    def mine(self):
        ...
        while self.mining:
            ...
            if time.time() - lp > 5:
                ...
                avg = 16 ** self.diff  # ~65,536 pour diff=4
                eta = "..."
...
╚══════════════════════════════════════════════════════════════╝{X}""")
    ...
    s = m.stats()
    if s:
        print(f"{G}[OK]{X} Hauteur: {s.get('height',0)} | Difficulté: {s.get('difficulty',4)}")
